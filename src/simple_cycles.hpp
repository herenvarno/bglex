...
vector<vector<typename G::vertex_descriptor>> simple_cycles(G g_) {
  ...
  typedef adjacency_list<vecS, vecS, directedS, property<vertex_color_t, int>,
  ...
  while (!sccs.empty()) {
    ...
    for (tie(oei, oei_end) = out_edges(sccG.global_to_local(startnode), sccG);
         oei != oei_end; oei++) {
    ...
    while (!q.empty()) {
      ...
      if (!nbrs.empty()) {
        ...
        if (nextnode == startnode) {
          ...
          for (tie(oei, oei_end) =
                   out_edges(sccG.global_to_local(nextnode), sccG);
               oei != oei_end; oei++) {
      ...
      if (nbrs.size() == 0) {
        if (closed.find(thisnode) != closed.end()) {
          ...
          for (tie(oei, oei_end) =
                   out_edges(sccG.global_to_local(thisnode), sccG);
               oei != oei_end; oei++) {
            Vertex nextnode = sccG.local_to_global(target(*oei, sccG));
